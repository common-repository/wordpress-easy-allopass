...
@define("URL_WES","http://www.resabe.com/");
// Plugin dir URL
@define("URL_WEA",plugin_dir_url( __FILE__ ));
// Plugin Dirname
@define("DIR_WEA",dirname(__FILE__)."/");
// Plugin Setup Instructions
@define("URL_HELP","http://www.resabe.com/wea-wp-easy-allopass-instructions-");
			...
			if (isset($_GET["ok"]) and isset($_GET["s"]) and $_GET["s"]==$id)
				{
					...
					if (isset($_COOKIE["POST_".$page."_".$id]) and wp_remote_retrieve_body(wp_remote_get(URL_WES . "wea-verif-hp.php?id=$id_allopass&return_url=".urlencode($return_url__)."&code=" . $_GET["ok"])) == 1)
						{
				...
				if ( isset($_COOKIE["POST_".$page."_".$id]))
						{
						if (wp_remote_retrieve_body(wp_remote_get(URL_WES . "wea-verif-hp.php?id=$id_allopass&code=" . $_COOKIE["POST_".$page."_".$id])) == 1)
							{
							$r_d = "<div style='color:#999999;background-color:#FFFF99;text-align:center;padding:5px;border:solid 1px #000000'><b><em>".__( '-- Start unlocked content --', 'wp-easy-allopass' )."</em></b></div><br>";
							$r_f = "<br><br><div style='color:#999999;background-color:#FFFF99;text-align:center;padding:5px;border:solid 1px #000000'><b><em>".__( '-- End unlocked content --', 'wp-easy-allopass' )."</em></b></div>";								
							$r = $r_d.do_shortcode($content).$r_f;
...
function WEA_allopass_admin() 
{
	...
	if ( isset($_POST) and !isset($_GET["act"]) and !isset($_POST["del"]) )
	{
		...
		if (isset($_POST["id_site"]) && isset($_POST["id_produit"]) )
		{
			if ( is_numeric($_POST["id_site"]) && is_numeric($_POST["id_produit"]) )
			{
			$id_pass = 	$_POST["id_site"].'/'.$_POST["id_produit"];
			$sql = "INSERT INTO ".TBL_PROD." (id_prod, prod_descr, last_update) VALUES ('".$id_pass."','".addslashes($_POST["txt_allopass"])."','".date('Ymd')."');";		
			$wpdb->query($sql);
			echo '<div id="message" class="updated highlight"><p>'.__( 'Product added! Ref:', 'wp-easy-allopass' ).' <strong>'.$id_pass.'</strong></p></div>';
			}
			else
			{
	...
	if (isset($_POST) and isset($_GET["act"]) and $_GET["act"] == "save")
	{
		...
		if (isset($_POST["id_site"]) && isset($_POST["id_produit"]) )
		{
			...
			{
			$id_pass = 	$_POST["id_site"].'/'.$_POST["id_produit"];
			$sql = "UPDATE ".TBL_PROD." SET id_prod='".$id_pass."', prod_descr='".addslashes($_POST["txt_allopass"])."', last_update='".date('Ymd')."' WHERE id='".$_GET["edit"]."';";		
			$wpdb->query($sql);
			echo '<div id="message" class="updated highlight"><p>'.__( 'Product Info updated! Ref :', 'wp-easy-allopass' ).' <strong>'.$id_pass.'</strong></p></div>';
			}
			else
			{
			echo '<div id="message" class="error"><p>'.__( 'Please enter a valid Product Id!', 'wp-easy-allopass' ).'</p></div>';
	...
	if (isset($_POST) and isset($_POST["del"]) and $_POST["del"] != "")
	{
		if (isset($_POST["del"]))
		{
			...
			$wpdb->query($sql);
			echo '<div id="message" class="updated highlight"><p>'.__( 'Product Ref :', 'wp-easy-allopass' ).' <strong>'.$_POST["id_produit"].' '.__( 'deleted!', 'wp-easy-allopass' ).'</strong></p></div>';
		}
	}	
	$id_allopass = "";
